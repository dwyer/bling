...
static expr_t *unary_expression(parser_t *p) {
    ...
    case token_DEC:
    case token_INC:
        parser_error(p, p->pos, fmt_sprintf("unary `%s` not supported in subc", token_string(p->tok)));
        return NULL;
    default:
...
static expr_t *type_specifier(parser_t *p) {
    ...
    case token_SIGNED:
    case token_UNSIGNED:
        parser_error(p, p->pos, fmt_sprintf("`%s` is not supported in subc", token_string(p->tok)));
        break;
    case token_STRUCT:
    case token_UNION:
    ...
    default:
        if (is_type(p)) {
            ...
            if (p->lit) {
                parser_error(p, p->pos, fmt_sprintf("expected type, got %s", p->lit));
            } else {
...
extern ast_File *parser_parse_cfile(const char *filename, scope_t *pkg_scope) {
