...
static void emit_type(emitter_t *e, expr_t *type);
...
static void emit_field(emitter_t *e, field_t *field) {
    if (field->type == NULL && field->name == NULL) {
        ...
        emit_type(e, field->type);
        if (field->name != NULL) {
            emit_space(e);
            emit_expr(e, field->name);
        }
    }
}
...
static void emit_expr(emitter_t *e, expr_t *expr) {
    ...
    case ast_TYPE_NAME:
        emit_type(e, expr->type_name.type);
        break;
...
static void emit_spec(emitter_t *e, spec_t *spec) {
    ...
    case ast_SPEC_TYPEDEF:
        ...
        emit_space(e);
        emit_type(e, spec->typedef_.type);
        emit_token(e, token_SEMICOLON);
        break;

    case ast_SPEC_VALUE:
        emit_token(e, token_VAR);
        if (spec->value.name) {
            emit_space(e);
            emit_expr(e, spec->value.name);
        }
        emit_space(e);
        emit_type(e, spec->value.type);
        if (spec->value.value) {
            emit_space(e);
            emit_token(e, token_ASSIGN);
            emit_space(e);
...
static void emit_decl(emitter_t *e, decl_t *decl) {
    ...
    case ast_DECL_FUNC:
        emit_token(e, token_FUNC);
        emit_space(e);
        emit_expr(e, decl->func.name);
        emit_type(e, decl->func.type);
        if (decl->func.body) {
            emit_space(e);
