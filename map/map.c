...
extern map$Map map$init(int val_size) {
    map$Map m = {
        .len = 0,
        .pairs = utils$init(sizeof(pair_t)),
        .key_size = sizeof(uintptr_t),
        .val_size = val_size,
    };
    utils$set_len(&m.pairs, default_cap);
    return m;
}

extern void map$deinit(map$Map *m) {
    // for (int i = 0; i < utils$len(&m->pairs); i++) {
    //     pair_t *p = (pair_t *)utils$ref(&m->pairs, i);
    // }
    utils$deinit(&m->pairs);
}
...
static pair_t *pair_ref(const map$Map *m, const void *key) {
    ...
    for (int i = 0; i < utils$len(&m->pairs); i++) {
        ...
        pair_t *p = (pair_t *)utils$ref(&m->pairs, idx);
...
extern void map$set(map$Map *m, const char *key, const void *val) {
    ...
    if (load_factor >= max_load_factor) {
        ...
        m->pairs = utils$init(m->pairs.size);
        utils$set_len(&m->pairs, new_cap);
        m->len = 0;
        for (int i = 0; i < utils$len(&pairs); i++) {
            pair_t *p = (pair_t *)utils$ref(&pairs, i);
            if (p->key) {
...
extern int map$iter_next(map$iter_t *m, char **key, void *val) {
    while (m->_idx < utils$len(&m->_map->pairs)) {
        pair_t *p = (pair_t *)utils$ref(&m->_map->pairs, m->_idx);
        m->_idx++;
        if (p->key) {
            if (key) {
                *(void **)key = p->key;
